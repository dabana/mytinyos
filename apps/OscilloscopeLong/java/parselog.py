...
for i in range(0, len(readings)):
    ...
    for j in range(0, len(sparsed_reading), 2):
    ...
    for j in range(1, len(sparsed_reading), 2):
        ...
        RHlin_sum += RH_lin
        RHtrue_sum += RH_true
        SORH_sum += SORH * 0.01 - 39.6

        #if index-19 divisible by 4
        if j == 9:
        
            relhum.append(RHlin_sum / 5)
            relhum_tcomp.append(RHtrue_sum / 5)
            rawhum.append(SORH_sum / 5)
...
l = [range(1,len(readings)+1), time, temp, relhum, relhum_tcomp]
